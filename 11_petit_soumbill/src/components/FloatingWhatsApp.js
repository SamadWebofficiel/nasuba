...
export default function FloatingWhatsApp() {
  ...
  const message = useUIStore((state) => state.whatsappMessage);

  useEffect(() => {
    setMounted(true);
    
    // Fetch global settings to get real whatsapp number if any
    async function fetchSettings() {
      try {
        ...
        if (res.ok) {
          ...
          if (data.fields) {
            setSettings({
              whatsapp: data.fields.whatsappNumber?.stringValue || "22900000000",
            });
          }
        }
      } catch (e) {
  ...
  return (
    <a 
      ...
      className="fixed bottom-6 right-6 z-50 bg-[#25D366] hover:bg-[#20bd5a] text-white w-14 h-14 rounded-full shadow-lg shadow-[#25D366]/40 flex items-center justify-center transition-transform hover:scale-110 active:scale-95"
